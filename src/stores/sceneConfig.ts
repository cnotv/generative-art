...
export const useSceneConfigStore = defineStore('sceneConfig', () => {
  const internalConfig = ref<Record<string, unknown>>(
    JSON.parse(JSON.stringify(GLOBAL_SCENE_DEFAULTS))
  )
  ...
  const updateValue = (path: string, value: unknown): void => {
    ...
    const parent = keys
      ...
      >((object, key) => (object as Record<string, unknown>)[key] as Record<string, unknown>, internalConfig.value as Record<string, unknown>)
...
export const unregisterSceneConfig = () => useSceneConfigStore().unregisterSceneConfig()
