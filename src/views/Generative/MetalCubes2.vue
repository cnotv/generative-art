...
import { animateTimeline, createLights, instanceMatrixMesh } from '@/utils/threeJs';
...
const init = (canvas: HTMLCanvasElement, statsEl: HTMLElement, ) => {
  ...
  const setup = () => {
    ...
    const cubes = instanceMatrixMesh(cubeMesh, scene, positions.map(position => ({ position, scale: [config.size, config.size, config.size] })));
    ...
    const animate = () => {
      ...
      animateTimeline([{ start: 0, action: () => camera.position.z += -1 }], animationId.value);
