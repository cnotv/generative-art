...
import { bindAnimatedElements, animateTimeline, createLights, getEnvironment, getGround, removeElements } from '@/utils/threeJs';
...
const init = async (canvas: HTMLCanvasElement, statsEl: HTMLElement, ) => {
  ...
  const setup = async () => {
    const { renderer, scene, camera, clock, orbit, world } = getEnvironment(canvas, { camera: { position: [0, 150, 0] } });
    ...
    const addBall = (position: CoordinateTuple) => {
      elements.push(getBall(scene, world, {position, size: 0.5, weight: 80, restitution: -0.5, color: 0xffffff, opacity: 0.5, castShadow: false, receiveShadow: false}));
    }
    ...
    function animate() {
      ...
      animateTimeline([{interval: [1, 300], action: () => (elements = removeElements(scene, world, elements))}], frame);
