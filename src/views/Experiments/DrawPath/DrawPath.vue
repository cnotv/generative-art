...
watch(
  () => reactiveConfig.value.showNodes,
  (show) => {
    if (!sceneState) return
    drawRemoveWaypointNodes(sceneState.scene, sceneState.waypointNodes)
    if (show && sceneState.controlWaypoints.length > 0) {
      sceneState.waypointNodes = sceneState.controlWaypoints.map((wp) =>
        drawCreateWaypointNode(
          sceneState!.scene,
          wp,
          sceneState!.waypointNodeGeo,
          sceneState!.waypointNodeMat
        )
      )
    } else {
      sceneState.waypointNodes = []
  ...
  if (reactiveConfig.value.playing && follower && state.isFollowing && state.followState) {
    ...
    if (mode === 'physics') {
      ...
      modelFollowApplyContactImpulses(
        ...
        effectiveSpeed,
        delta,
        reactiveConfig.value.obstacleImpulse
      )
    } else if (mode === 'goomba') {
      ...
      const result = modelFollowPhysicsTick(
        follower,
        state.followState,
        effectiveSpeed,
        delta,
...
const init = async (): Promise<void> => {
  ...
  if (ambientLight) {
    registerLightProperties({
      light: ambientLight as unknown as {
        color: { getHex: () => number; set: (v: number) => void }
        intensity: number
      },
      name: 'ambient-light',
      title: 'Ambient Light'
  ...
  if (directionalLight) {
    registerLightProperties({
      light: directionalLight as unknown as {
        color: { getHex: () => number; set: (v: number) => void }
        intensity: number
      },
