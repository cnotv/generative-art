 ...
 */
export const drawInterpolateWaypoints = (waypoints: Waypoint[]): Waypoint[] => {
...
export const drawRemoveWaypointNodes = (scene: THREE.Scene, nodes: THREE.Mesh[]): void => {
  nodes.forEach((mesh) => scene.remove(mesh))
}

/** Update the world position of a single node mesh to match a moved waypoint. */
export const drawUpdateWaypointNodePosition = (node: THREE.Mesh, waypoint: Waypoint): void => {
  node.position.set(waypoint.x, waypoint.y, waypoint.z)
}

export const drawRemovePathVisualization = (scene: THREE.Scene, group: THREE.Group): void => {
  scene.remove(group)
