...
const dot2 = (gradient: number[], x: number, y: number): number => gradient[0] * x + gradient[1] * y
 ...
 */
export const fractalNoise = (x: number, z: number, config: NoiseConfig): number => {
  const { seed, octaves, frequency, amplitude, lacunarity, persistence } = config

  const totalNoise = Array.from({ length: octaves }).reduce<{
    value: number
