...
const initializeWorld = async (scene: THREE.Scene, world: Parameters<typeof getModel>[1]) => {
  ...
  sharedGrassGeometry = createGrassBladeGeometry(
    (reactiveConfig.value.grassBladeScale as number | undefined) ?? GRASS_BLADE_SCALE
  )
  sharedGrassMaterial = createGrassMaterial(
    (reactiveConfig.value.grassColor as number | undefined) ?? GRASS_COLOR
  )
...
const updateDirectionalLight = (light: THREE.DirectionalLight, target: THREE.Object3D): void => {
  light.position.set(
...
const snapToTerrain = (
  ...
  raycaster.ray.origin.set(playerMesh.position.x, TERRAIN_RAYCAST_ORIGIN_Y, playerMesh.position.z)
...
const applyTerrainSnap = (): void => {
  ...
  if (worldCase === 'terrain' || worldCase === 'all') {
    ...
    snapToTerrain(
      ...
      chunkSize,
      terrainRaycaster,
      playerBottomOffsetReference ?? 0
    )
  }
  if (directionalLightReference)
    updateDirectionalLight(directionalLightReference, playerMeshReference)
}
...
const applyCameraFollow = (): void => {
  ...
  cameraFollowPlayer(
    ...
    playerMeshReference,
    CAMERA_OFFSET,
    orbit as Parameters<typeof cameraFollowPlayer>[3],
    ['x', 'z']
  )
  ...
  const cameraYStep =
    cameraYDiff * Math.abs(cameraYDiff) * CAMERA_Y_LERP_SPEED * getDeltaReference()
  cameraReference.position.y +=
    Math.abs(cameraYStep) > Math.abs(cameraYDiff) ? cameraYDiff : cameraYStep
  store.cameraConfig = {
    ...(store.cameraConfig as Record<string, unknown>),
    position: {
      x: cameraReference.position.x,
      y: cameraReference.position.y,
      z: cameraReference.position.z
    }
  }
}
...
const applyPlayerMove = (
  ...
  const followFrequency =
    (reactiveConfig.value.cameraFollowFrequency as number | undefined) ?? CAMERA_FOLLOW_FREQUENCY
  if (cameraFrameCounter % followFrequency === 0) applyCameraFollow()
}

const getViewZOffset = (): number =>
  -((reactiveConfig.value.forwardBias as number | undefined) ?? FORWARD_BIAS)
...
const buildGeneratorConfig = () => {
  ...
  return {
    ...
    ...(grassDensity !== undefined
      ? { grassPerChunk: grassDensity * GRASS_DENSITY_MULTIPLIER }
      : {}),
...
const spawnVisibility = computed(
...
const getGroundColor = (worldCase: WorldCase): number => {
  const treesColor =
    (reactiveConfig.value.treesGroundColor as number | undefined) ?? TREES_GROUND_COLOR
  const grassColor =
    (reactiveConfig.value.grassGroundColor as number | undefined) ?? GRASS_GROUND_COLOR
...
const buildChunkOptions = () => ({
  ...
  terrainBaseColor:
    (reactiveConfig.value.terrainBaseColor as number | undefined) ?? TERRAIN_BASE_COLOR,
  terrainPeakColor:
    (reactiveConfig.value.terrainPeakColor as number | undefined) ?? TERRAIN_PEAK_COLOR,
  treeSizeScale: (reactiveConfig.value.treeSizeScale as number | undefined) ?? TREE_SIZE_SCALE,
  treeSizeVariation:
    (reactiveConfig.value.treeSizeVariation as number | undefined) ?? TREE_SIZE_VARIATION,
  spawnId: SPAWN_ID,
  ...
  elementPropertiesStore.registerElementProperties(SPAWN_ID_TERRAIN, {
    ...
    getValue: (path) => {
      if ((TERRAIN_COLOR_KEYS as readonly string[]).includes(path))
        return reactiveConfig.value[path]
    ...
    updateValue: (path, value) => {
      if ((TERRAIN_COLOR_KEYS as readonly string[]).includes(path)) {
        ...
        reactiveConfig.value = {
          ...reactiveConfig.value,
          procedural: setNestedValueImmutable(
            reactiveConfig.value.procedural as Record<string, unknown>,
            path,
            value
          )
        }
...
watch(
  () => reactiveConfig.value.worldCase,
  (worldCase) => {
    updateViewSchema(route.name as string, buildConfigSchema(worldCase as WorldCase))
    if (sceneReference && activeChunksReference && sharedGrassGeometry && sharedGrassMaterial) {
      applyWorldCaseToAllChunks(
        activeChunksReference,
        buildApplyWorldCaseOptions(worldCase as WorldCase)
      )
    }
...
watch(
  () => reactiveConfig.value.grassColor,
  (newColor) => {
    if (sharedGrassMaterial) {
      ;(sharedGrassMaterial as THREE.MeshPhysicalMaterial).color.set(
        (newColor as number | undefined) ?? GRASS_COLOR
      )
    }
  }
)
watch(
  () => reactiveConfig.value.grassBladeScale,
  () => {
...
onMounted(async () => {
  ...
  await store.init(canvas.value, setupConfig, {
    ...
    defineSetup: async ({ scene, camera, world, getDelta, animate }) => {
      const { playerMesh, activeChunks, directionalLight, playerBottomOffset } =
        await initializeWorld(scene, world)
      ...
      timelineManager.addAction({
        ...
        action: () => {
          ...
          const moveSpeed =
            ((reactiveConfig.value.movementSpeed as number | undefined) ?? game.distance) *
            MOVEMENT_SPEED_SCALE
          if (isFreeCamera) {
      ...
      timelineManager.addAction({
        ...
        action: () => {
          const isFreeCamera = reactiveConfig.value.freeCamera as boolean
          const currentViewRadius =
            (reactiveConfig.value.viewRadius as number | undefined) ?? VIEW_RADIUS
          updateChunks({
            playerPosition: isFreeCamera
              ? (cameraReference?.position ?? playerMesh.position)
              : playerMesh.position,
            activeChunks,
            scene,
            generatorConfig: buildGeneratorConfig(),
