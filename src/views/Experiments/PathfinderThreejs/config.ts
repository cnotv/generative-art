...
export const sceneControls = {
  ...
  lights: {
    ambient: {
      intensity: { min: 0, max: 5, step: 0.1, label: 'Ambient Intensity', sectionStart: true }
    },
...
export const buildConfigControls = (hasStart: boolean, hasGoal: boolean) => ({
  ...
  brush: {
    ...
    options: configControls.brush.options.map((opt) =>
      opt.value === 'start'
        ? { ...opt, disabled: hasStart }
        : opt.value === 'goal'
          ? { ...opt, disabled: hasGoal }
          : opt
    )
  }
})
...
export const scenarios: Record<string, ScenarioConfig> = {
  ...
  'zig-zag': {
    ...
    start: REF_START,
    goal: REF_GOAL,
    cells: [...col(2, 10), ...colFrom(5, 1, 10), ...col(8, 10)]
  },
