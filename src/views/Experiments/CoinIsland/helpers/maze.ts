...
const removeWallBetween = (current: MazeCell, neighbor: MazeCell): void => {
  ...
  if (rowDiff === -1) { current.walls.north = false; neighbor.walls.south = false; }
  if (rowDiff === 1) { current.walls.south = false; neighbor.walls.north = false; }
  if (colDiff === -1) { current.walls.west = false; neighbor.walls.east = false; }
  if (colDiff === 1) { current.walls.east = false; neighbor.walls.west = false; }
};
