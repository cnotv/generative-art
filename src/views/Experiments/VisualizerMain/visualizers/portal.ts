...
const calculateWaveDeformation = (
  ...
  const waves = WAVE_CONFIGS[bandType].map((config, index) => {
    ...
    return (
      waveFunction(baseFreq + timeComponent + audioComponent + vComponent) *
      audioValue *
      config.amplitude
    )
  })
 ...
 */
const createFrequencyBandMaterial = (
...
const getProcessedAudioBands = (): { [key: string]: number } => {
  ...
  const sourceMultiplier =
    source === 'song' ? config.sourceMultiplier.song : config.sourceMultiplier.mic
