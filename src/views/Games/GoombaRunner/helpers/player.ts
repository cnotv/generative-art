...
const createPlayer = async (scene: THREE.Scene, physics: any, world: RAPIER.World) => {
  ...
  const colliderDesc = physics.RAPIER.ColliderDesc.capsule(capsuleHeight / 2, capsuleRadius)
...
const handleJump = (
  ...
  if (config.player.jump.isActive) {
    ...
    if (jumpProgress >= 1.0) {
      ...
      const targetY = player.userData.baseY + config.player.jump.height * jumpCurve
...
const movePlayer = (
  ...
  if (physics && playerController) {
    ...
    playerController.computeColliderMovement(player.userData.collider, moveVector)
...
const handleArcMovement = (player: ComplexModel) => {
  ...
  const currentY = startPos.y + (targetPos.y - startPos.y) * easeProgress + arcHeight
