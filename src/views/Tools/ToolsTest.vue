...
const cameraSideBindings = Object.values(CameraSide).reduce((accumulator, preset) => {
  ...
  <ControlsLogger v-if="gameState" :logs="logs">
    <div>
      ...
      <span
        >-
        {{
          currentActions['toggle-move'] ? 'Free move' : `In the loop (${gameState.data.score || 0})`
        }}</span
      >
    </div>
  </ControlsLogger>
