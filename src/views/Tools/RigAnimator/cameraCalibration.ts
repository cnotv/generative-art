...
import { measureHeadOrientation, measureTorsoOrientation } from './cameraBodyOrientation'
...
const toQuaternionData = ({ x, y, z, w }: Quaternion): QuaternionData => ({ x, y, z, w })
...
/**
 * Read the reference sizes and orientations of a held front T-pose.
...
export const measureFrontCalibration = (
  ...
  const torsoOrientation = measureTorsoOrientation(frame.world)
  if (
    !leftShoulder ||
    !rightShoulder ||
    !leftWrist ||
  ...
  if (shoulderSpanImage < MINIMUM_EXTENT || shoulderWidthMeters < MINIMUM_EXTENT) return null
  const headOrientation = measureHeadOrientation(frame.world)
  return {
    shoulderSpanImage,
    ...
    torsoOrientation: toQuaternionData(torsoOrientation),
    headOrientation: headOrientation && toQuaternionData(headOrientation),
...
/**
 * How far the skeleton root should move from rest, read from a live frame against the calibration.
 ...
 * @returns The offset, or null without a front calibration or detected shoulders
 */
export const computeCalibratedRootOffset = (
  frame: CalibrationFrame,
  calibration: CameraCalibration,
  rigShoulderWidthWorld: number,
  options: CalibratedRootOffsetOptions
): Vector3Data | null => {
  const { front } = calibration
  const leftShoulder = visiblePoint(frame, LANDMARK_INDEX.leftShoulder)
  const rightShoulder = visiblePoint(frame, LANDMARK_INDEX.rightShoulder)
