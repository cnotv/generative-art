...
const snapshotGroupConfig = (): GroupConfig => ({
  ...
  textures: {
    ...
    rotationVariation: [...reactiveConfig.value.textures.rotationVariation] as CoordinateTuple
  },
...
const loadGroupConfig = (groupConfig: GroupConfig) => {
  ...
  reactiveConfig.value.textures.baseSize = [...groupConfig.textures.baseSize] as CoordinateTuple
...
const applyGroundUpdate = (field: string, value: unknown) => {
  ...
  if (field === 'color' && currentGround?.mesh?.material) {
    ;(currentGround.mesh.material as THREE.MeshStandardMaterial).color = new THREE.Color(
      value as number
    )
  }
}
...
const registerGroupElementProperties = (group: TextureGroup) => {
  registerElementProperties(group.id, {
    ...
    getValue: (path) =>
      getNestedValue(reactiveConfig.value as unknown as Record<string, unknown>, path),
...
const handleFileUpload = (event: Event) => {
  ...
  if (files && files.length > 0) {
    ...
    nextTick(() => {
      if (currentScene && currentWorld)
        addGroupMeshes(currentScene, currentWorld, newGroup, getGroupConfig, () =>
          updateSceneElements(currentScene)
        )
    })
...
const initScene = async () => {
  ...
  registerCameraHandlers(
    ...
    ],
    {
      onPresetChange: (_slotId, preset) => {
        ...
        const currentType =
          currentCamera instanceof THREE.PerspectiveCamera ? 'perspective' : 'orthographic'
  ...
  registerElementProperties(cameraLabel, {
    ...
    schema: cameraSchema,
    getValue: (path) =>
      getNestedValue(activeSlot.value as unknown as Record<string, unknown>, path),
  ...
  if (groundConfig.value.enabled) {
    registerElementProperties('ground', {
      ...
      schema: groundSchema,
      getValue: (path) =>
        getNestedValue(groundConfig.value as unknown as Record<string, unknown>, path),
      updateValue: (path, value) =>
        updateGroundField(path as keyof typeof groundConfig.value, value)
    })
  }
  ...
  if (skyConfig.value.enabled) {
    registerElementProperties('sky', {
      ...
      schema: skySchema,
      getValue: (path) =>
        getNestedValue(skyConfig.value as unknown as Record<string, unknown>, path),
  ...
  registerElementProperties('lights', {
    ...
    schema: lightsSchema,
    getValue: (path) =>
      getNestedValue(lightsConfig.value as unknown as Record<string, unknown>, path),
