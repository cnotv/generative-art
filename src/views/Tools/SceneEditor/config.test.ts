...
describe('SceneEditor element schemas — Properties panel contract', () => {
  ...
  describe('all standard element schemas are non-empty', () => {
    it.each(
      Object.entries(STANDARD_ELEMENT_SCHEMAS) as [StandardElementKey, Record<string, unknown>][]
    )('"%s" schema has at least one property', (_name, schema) => {
  ...
  describe('camera schema controls cover expected fields', () => {
    ...
    it('orbitTarget is a CoordinateInput', () => {
      expect((cameraSchema.orbitTarget as Record<string, unknown>).component).toBe(
        'CoordinateInput'
      )
    })
  })
  ...
  describe('Properties panel never shows empty content for standard elements', () => {
    ...
    it.each(
      Object.entries(STANDARD_ELEMENT_SCHEMAS) as [StandardElementKey, Record<string, unknown>][]
    )('"%s" element has non-empty activeProperties.schema when selected', (name, schema) => {
      // Arrange
