    ...
    onMounted(() => {
      ...
      registerElementProperties('Camera', {
        ...
        updateValue: (path, value) => {
          if (path === 'fov') camera.fov = value as number
        }
      })
      ...
      registerElementProperties('ambient-light', {
        ...
        updateValue: (path, value) => {
          if (path === 'intensity') ambientLight.intensity = value as number
        }
      })
      ...
      registerElementProperties('directional-light', {
        ...
        updateValue: (path, value) => {
          if (path === 'intensity') directionalLight.intensity = value as number
        }
      })
      ...
      registerElementProperties('ground', {
        ...
        getValue: (path) =>
          path === 'color' ? `#${groundMaterial.color.getHexString()}` : undefined,
        updateValue: (path, value) => {
          if (path === 'color') groundMaterial.color.set(value as string)
        }
      })
      ...
      registerElementProperties('sky', {
        ...
        updateValue: (path, value) => {
          if (path === 'color') skyMaterial.color.set(value as string)
        }
      })
      ...
      registerElementProperties('TestCube', {
        ...
        getValue: (path) =>
          path === 'color' ? `#${cubeMaterial.color.getHexString()}` : undefined,
        updateValue: (path, value) => {
          if (path === 'color') cubeMaterial.color.set(value as string | number)
        }
      })
    })
...
describe('ElementsPanel - E2E browser test', () => {
  ...
  it.each(EXPECTED_ELEMENT_NAMES.map((name) => [name] as [ExpectedElementName]))(
    'clicking "%s" expands its properties section',
    async (elementName) => {
      const panelContent = getSheetContent()
      expect(panelContent).not.toBeNull()
...
describe('ElementsPanel - scene elements reactivity', () => {
  ...
  it('panel updates reactively when sceneElements are added', async () => {
    ...
    debugStore.registerSceneElements({ type: 'PerspectiveCamera' }, [
...
describe('ElementsPanel - texture upload handler', () => {
  ...
  it('clicking Add Texture Area and selecting a file calls onAddNewGroup handler', async () => {
    ...
    const mockInput = {
      type: '',
      accept: '',
      set onchange(handler: (e: Event) => void) {
        ...
        textureStore.groups = [
          {
            id: GROUP_ID,
            name: GROUP_NAME,
            textures: [{ id: 't1', name: 'tex.png', filename: 'tex.png', url: 'blob:fake' }]
          }
        ]
        debugStore.setSceneElements(
