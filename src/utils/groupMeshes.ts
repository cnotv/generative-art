    ...
    .flatMap((group) => {
      ...
      const positionsByVariant = allPositions.reduce<CoordinateTuple[][]>(
        (accumulator, position) => {
          ...
          return accumulator.map((positions, i) =>
            i === randomIndex ? [...positions, position] : positions
          )
        },
        group.textures.map(() => [])
      )
...
export const addGroupMeshes = (
  ...
  variantData.forEach((variant) => {
    ...
    elementsData.forEach((elementData, index) => {
      const meshSize =
        (elementData as { scale?: CoordinateTuple }).scale || variant.groupConfig.textures.baseSize
      const meshPosition = elementData.position
      const meshRotation =
        (elementData as { rotation?: CoordinateTuple }).rotation || ([0, 0, 0] as CoordinateTuple)
