...
const actions = await getAnimations(mixer, ['animations/running.fbx', 'animations/present.json'])
...
`scripts/generate-present-animation.mjs`, which solves a gesture against the Mixamo skeleton
and writes it next to the FBX clips.
