...
A labelled undo stack, generic over whatever an editor uses as a snapshot, with an action log the editor can show.
...
const painted = historyPush(opened, 'Deleted keyframe', nextKeyframes)
...
Every action taken, newest first, as `{ label, undone }`. The opening state is left out: it is where undo lands, not something anybody did.
