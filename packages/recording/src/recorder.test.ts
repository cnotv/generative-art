...
const createMockMediaRecorderClass = () => {
  ...
  const MockMediaRecorder = vi.fn(function (this: MockMediaRecorderInstance) {
    this.state = 'inactive'
...
describe('recordCreate', () => {
  ...
  describe('onStateChange', () => {
    it('should notify listeners when state changes', () => {
      ...
      expect(callback).toHaveBeenCalledWith(expect.objectContaining({ isRecording: true }))
    ...
    it('should notify on elapsed time updates', () => {
      ...
      expect(callback).toHaveBeenCalledWith(expect.objectContaining({ elapsed: 100 }))
  ...
  describe('format support', () => {
    it.each([
      ['webm', 'video/webm'],
      ['mp4', 'video/mp4']
    ] as const)('should create MediaRecorder with %s format', (format, mimeType) => {
