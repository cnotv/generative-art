...
const getCellType = (pos: Position2D, grid: Grid): CellType => grid.cells[pos.z][pos.x].type
...
const reconstructPath = (node: PathNode): Position2D[] =>
  node.parent === null ? [node.position] : [...reconstructPath(node.parent), node.position]
...
const getNeighborNodes = (
  ...
  const orthogonal = ORTHOGONAL_NEIGHBORS.map((offset) => ({
    x: current.position.x + offset.x,
    z: current.position.z + offset.z
  }))
    .filter((pos) => isWalkable(pos, grid) && !closedSet.has(positionKey(pos)))
...
const updateOpenList = (openList: PathNode[], neighbors: PathNode[]): PathNode[] =>
...
export const logicGetBestRoute = (
  ...
  return Array.from({ length: maxIterations }).reduce<Position2D[] | null>((result) => {
